  ...
  -- 覆盖 LazyVim 的 clangd 配置
  {
    ...
    opts = function(_, opts)
      ...
      opts.servers.clangd = vim.tbl_deep_extend("force", opts.servers.clangd or {}, {
        cmd = {
          -- 使用 Mason 安装的 clangd 完整路径
          vim.fn.stdpath("data") .. "/mason/packages/clangd/clangd_21.1.0/bin/clangd.exe",
          "--background-index",
          "--clang-tidy",
          ...
          "--fallback-style=llvm",
          -- 使用 query-driver 让 clangd 自动发现编译器的系统头文件（修正为 mingw64）
          "--query-driver=C:/msys64/mingw64/bin/gcc.exe,C:/msys64/mingw64/bin/g++.exe",
        },
        capabilities = {
          offsetEncoding = { "utf-16" },
        },
        -- 设置初始化选项，添加编译标志（修正为 mingw64 路径）
        init_options = {
